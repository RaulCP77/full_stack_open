...
describe('addition of a new blog', () => {
    ...
    test('a valid blog can be added', async () => {
        ...
        await user.save()
        const updatedUser = await User.findById(userId).populate('blogs', { title: 1, author: 1, url: 1, likes: 1 })
        assert.strictEqual(updatedUser.blogs.length, 1, 'Blog was not added to user')
    })
    ...
    test('adding a blog without title or url returns 400', async () => {
        ...
        const response = await api.get('/api/blogs')
        const titles = response.body.map(blog => blog.title)
        assert(!titles.includes(newBlog.title), 'Blog without title or url should not be added')
    })
    ...
    test('a blog can be deleted', async () => {
        ...
        assert.strictEqual(blogsAtEnd.length, blogsAtStart.length - 1, 'Blog was not deleted')
    })
    ...
    test('a blog can be updated', async () => {
        ...
        assert.strictEqual(updatedBlogInDb.likes, updatedBlog.likes, 'Blog likes were not updated')
    })
})
